...
public class HarmonicLabels implements LocationMonitorAgent {
	private static final double epsilon = 1.0E-3;
	private static final boolean test = false;
	private static final boolean infPop = true;
...
//	private double[] newWeights;
	...
	public double[] calcWeights() {
		...
		oldLabels = newLabels;
		int iterations = 0;
		while (diff * getMag(oldLabels) > epsilon && iterations < 100) {
...
//				newLabels.put(vertex, new Double(numerator / denominator));
				newLabels.put(vertex, new Double(numerator));
			}
		}
		EpiSimUtil.normalize(newLabels);
		return newLabels;
	}
...
//			System.out.println(diff);
	...
	public static void getMagTest() {
		HashMap<Integer,Double> map = new HashMap<Integer,Double>();
		int numEntries = 2;
		for (int i=0; i < numEntries; i++) {
			Integer key = new Integer(i);
			Double value = new Double(1.0 / numEntries);
			map.put(key, value);
			System.out.println(value);
		}
		System.out.println(getMag(map));
	}
